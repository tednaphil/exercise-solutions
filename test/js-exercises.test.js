...
describe("JavaScript Exercises", () => {
  ...
  describe("PaliSum", () => {
    it("returns an array of twenty-five integers where the number plus its inverse equals a palindrome greater than 1,000", () => {
      ...
      expect(arrayONumbers).to.deep.equal([
        209, 308, 407, 506, 605, 704, 803, 902, 1000, 1001, 1002, 1003, 1004,
  ...
  describe("Classify - Perfect Numbers", () => {
    describe("Invalid Inputs", () => {
      it("Zero is rejected (not a natural number)", () => {
        expect(() => {
          classify(0);
        }).to.throw("Classification is only possible for natural numbers.");
      });
      it("Negative integer is rejected (not a natural number)", () => {
        expect(() => classify(-1)).to.throw(
  ...
  describe("ProteinTranslation", () => {
    ...
    describe("Multiple codons", () => {
      it("Sequence of two protein codons translates into proteins", () => {
        expect(translate("UUUUUU")).to.deep.equal([
          "Phenylalanine",
          "Phenylalanine",
        ]);
      });
      ...
      it("Translation stops if STOP codon at end of three-codon sequence", () => {
        expect(translate("AUGUUUUAA")).to.deep.equal([
          "Methionine",
          "Phenylalanine",
        ]);
      });
  ...
  describe("Pizza Order", () => {
    ...
    describe("Pizza Order Prices", () => {
      it("Returns correct price for single pizza order", () => {
        const order = { pizza: "Caprese", extras: [] };
        expect(orderPrice([order])).to.equal(9);
      });
      it("Returns correct price for pizza order with extras", () => {
        const margherita = { pizza: "Margherita", extras: [] };
        const caprese = { pizza: "Caprese", extras: ["ExtraToppings"] };
        expect(orderPrice([margherita, caprese])).to.equal(18);
        expect(orderPrice([caprese, margherita])).to.equal(18);
      });
      it("Returns correct price for a large and varied order", () => {
        const margherita = { pizza: "Margherita", extras: ["ExtraSauce"] };
        const caprese = {
          pizza: "Caprese",
          extras: ["ExtraToppings", "ExtraSauce"],
        };
        const caprese2 = { pizza: "Caprese", extras: ["ExtraToppings"] };
        const formaggio = {
          pizza: "Formaggio",
          extras: ["ExtraToppings", "ExtraToppings"],
        };
        const formaggio2 = { pizza: "Formaggio", extras: ["ExtraSauce"] };
        const formaggio3 = {
          pizza: "Formaggio",
